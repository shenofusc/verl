...
if is_package_present("torch_npu"):
    import torch_npu
...
if is_flash_attn_2_available():
    ...
    # flash-attn can't install on npu, so we need copy the logic to here.
...
class ParallelLlamaAttentionRmPad(ParallelLlamaAttention):
    ...
    def forward(self,
        ...
        if is_flash_attn_2_available():
            ...
            query_states, key_states = apply_rotary_pos_emb_rmpad(
                query_states,
                key_states,
                cos,
                sin,
        ...
        if is_flash_attn_2_available():
        ...
        elif is_package_present("torch_npu"):
            attention_mask_npu = torch.from_numpy(np.triu(np.ones([max_seqlen_in_batch, max_seqlen_in_batch]), k=1)).to(bool).npu()
            ...
            RuntimeError("No flash-attn kernel available, install flash-attn on gpu on install torch_npu on npu.")
